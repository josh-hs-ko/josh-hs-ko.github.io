...
Homotopically, we might think of this equality as symbolising the picture where $f\ x\ z$ can be continuously deformed to $f\ x\ y$ along any path $q : z =_A y$.
...
$$ \Pi(x\ z\ y : A)\ \Pi(q : z =\_A y)\ q = (f\ x\ z)^{-1} \cdot f\ x\ y $$

This changes the orientation of the homotopy, which I find intuitive but am not absolutely certain about; fortunately, though, this step is backed by the path-calculating techniques developed in Chapter 2 of the HoTT book (and demonstrates that those techniques are worth learning).
Finally we ‘pull’ $z$ to make it coincide with $x$ (that is, we substitute $x$ for $z$) and obtain

$$ \Pi(x\ y : A)\ \Pi(q : x =\_A y)\ q = (f\ x\ x)^{-1} \cdot f\ x\ y $$

which says that every $x =\_A y$ is contractible with $(f\ x\ x)^{-1} \cdot f\ x\ y$ as the center.