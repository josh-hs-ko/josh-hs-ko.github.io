...
Then the operators $M_i \defeq |i\rangle\langle i|$ together form a measurement $M$ since $\sum_i |i\rangle\langle i| = I$.
Now we reason:
...
& \mathsf{mixed}^D\ \rho\ (M, \mathit{ms}) = \\{\\, M_m\rho M_m^\dagger \mid m \in \mathit{ms} \\,\\}
\end{align*}
