...
*Deutsch’s problem* is (artificially) designed to make the sums useful: assuming that $f$ is either *constant* (always producing the same output) or *balanced* (producing two $|0\rangle$ and two $|1\rangle$ as the outputs — in general the same number of $|0\rangle$ and $|1\rangle$), the aim is to decide which is the case.
It is immediately clear that a constant $f$ —let’s write this as $f_{\mathsf c}$— can be distinguished from a balanced $f$ —let’s write this as $f_{\mathsf b}$— by applying them to $|{++}\rangle$ and inspecting the output:
...
That is, when applied to $|{++}\rangle$, $f_{\mathsf c}$ produces $|0\rangle$ or $|1\rangle$ (multiplied by $4$, which is omitted); for $f_{\mathsf b}$, the output sum consists of exactly two $|0\rangle$ and two $|1\rangle$, and is essentially $|+\rangle$.

In fact, we can ask a very succinct question to decide whether $f$ is $f_{\mathsf c}$ or $f_{\mathsf b}$: if we apply $f$ to $|{++}\rangle$, will the output ‘contain’ $|-\rangle$?
...
So the answer can be computed simply by checking whether $\langle-|f|{++}\rangle$ is non-zero or not.
...
It’s kind of expensive to compute $\langle-|f|{++}\rangle$ classically though (especially when the domain of $f$ is generalised from $\mathbf 2^2$ to $\mathbf 2^n$), and we want a more efficient quantum solution.

To proceed, we need a bit more analysis of $f$ with respect to the whole X-basis, not just $|{++}\rangle$.
For $f_{\mathsf c}$, its behaviour is very different when applied to the X-basis elements other than $|{++}\rangle$ — because the numbers of plus and minus signs are equal and everything is cancelled out, the output simply vanishes!
...
For $f_{\mathsf b}$, applying it to the other X-basis elements produces a $\pm 1$-weighted sum of two $|0\rangle$ and two $|1\rangle$.
...
That is, applying $f_{\mathsf b}$ to any X-basis element other than $|{++}\rangle$ will produce either $|-\rangle$ or nothing.

To solve Deutsch’s problem efficiently with a quantum computer, the Deutsch–Jozsa algorithm asks a clever question, which is similar to the previous one but involves some kind of *backward evaluation* of $f$: among the X-basis elements, what is a possible input that allows $f$ to produce an output that contains $|-\rangle$?
...
Even without knowing the ZX-calculus, we know that linear maps (or matrices) can be transposed, thereby switching their domain and codomain, so we can transpose the number $\langle-|f|{++}\rangle$ to $\langle{++}|f^{\mathrm T}|-\rangle$ and interpret the latter as answering ‘whether $f^{\mathrm T}|-\rangle$ contains $|{++}\rangle$’.
...
Applying the transpose to $|-\rangle$ (represented as a one-output Z-spider with phase $\pi$ below) gives us a quantum state representing the possible inputs that are mapped to $|-\rangle$ by $f$.
