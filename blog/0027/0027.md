...
It’s kind of expensive to compute this number classically though (especially when the domain of $f$ is generalised from $\mathbf 2^2$ to $\mathbf 2^n$).

To solve the problem efficiently with a quantum computer, the Deutsch–Jozsa algorithm asks a similar and clever question, which involves some kind of *backward evaluation* of $f$ on the X-basis: what is a possible input (among the X-basis elements) that allows $f$ to produce an output that contains $|-\rangle$?
