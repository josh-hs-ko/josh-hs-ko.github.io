...
Originally $\bar f$ is defined by its behaviour on the ‘Z-basis’ (or ‘computational basis’) $\\{|00\rangle, |01\rangle, |10\rangle, |11\rangle\\}$.
...
*Deutsch’s problem* is (artificially) designed to make the sums useful: assuming that $f$ is either *constant* (always producing the same output) or *balanced* (producing two $0$ and two $1$ as the outputs — in general the same number of $0$ and $1$), the aim is to decide which is the case.
It is immediately clear that a constant $f$ —let’s write this as $f_{\mathsf c}$— can be distinguished from a balanced $f$ —let’s write this as $f_{\mathsf b}$— by applying their linear version to $|{++}\rangle$ and inspecting the output:

\begin{align*}
\bar f_{\mathsf c}\\;|{++}\rangle &\approx |0\rangle \mathrel{\text{or}} |1\rangle \\\\
\bar f_{\mathsf b}\\;|{++}\rangle &\approx |+\rangle
\end{align*}

That is, when applied to $|{++}\rangle$, $\bar f_{\mathsf c}$ produces $|0\rangle$ or $|1\rangle$ (multiplied by $4$, which is omitted); for $\bar f_{\mathsf b}$, the output sum consists of exactly two $|0\rangle$ and two $|1\rangle$, and is essentially $|+\rangle$.

In fact, we can ask a very succinct question to decide whether $f$ is $f_{\mathsf c}$ or $f_{\mathsf b}$: if we apply $\bar f$ to $|{++}\rangle$, will the output ‘contain’ $|-\rangle$?
...
So the answer can be computed simply by checking whether $\langle-|\bar f|{++}\rangle$ is non-zero or not.
If $f$ is $f_{\mathsf c}$, then the output —be it $|0\rangle$ or $|1\rangle$— will contain $|-\rangle$; otherwise, if $f$ is $f_{\mathsf b}$, then the output will be $|+\rangle$, which does not contain $|-\rangle$.
It’s kind of expensive to compute $\langle-|\bar f|{++}\rangle$ classically though (especially when the domain of $f$ is generalised from $\mathbf 2^2$ to $\mathbf 2^n$), and we want a more efficient quantum solution.

To proceed, we need a bit more analysis of $\bar f$ with respect to the whole X-basis, not just $|{++}\rangle$.
For $\bar f_{\mathsf c}$, its behaviour is very different when applied to the X-basis elements other than $|{++}\rangle$ — because the numbers of plus and minus signs are equal and everything is cancelled out, the output simply vanishes!
...
For $\bar f_{\mathsf b}$, applying it to the other X-basis elements produces a $\pm 1$-weighted sum of two $|0\rangle$ and two $|1\rangle$.
...
$$ \bar f_{\mathsf b}\\;\\_ \approx |-\rangle \mathrel{\text{or}} 0 $$

That is, applying $\bar f_{\mathsf b}$ to any X-basis element other than $|{++}\rangle$ will produce either $|-\rangle$ or nothing.

To solve Deutsch’s problem efficiently with a quantum computer, the Deutsch–Jozsa algorithm asks a clever question, which is similar to the previous one but involves some kind of *backward evaluation* of $\bar f$: among the X-basis elements, what is a possible input that allows $\bar f$ to produce an output that contains $|-\rangle$?
...
The target state $|-\rangle$ is chosen so as to exclude $|{++}\rangle$ from the domain of $\bar f_{\mathsf b}$, which then becomes disjoint from the domain of $\bar f_{\mathsf c}$.

The backward question may be somewhat surprising because it doesn’t seem very natural, and it doesn’t seem easy to find an answer either.
This is where the insight from the ZX-calculus starts to come in: quantum maps are somewhat ‘directionless’ (which is reminiscent of relational programming), so backward evaluation is sometimes possible.
Even without knowing the ZX-calculus, we know that linear maps (or matrices) can be transposed, thereby switching their domain and codomain, so we can transpose the number $\langle-|\bar f|{++}\rangle$ to $\langle{++}|\bar f^{\mathrm T}|-\rangle$ and interpret the latter as answering ‘whether $\bar f^{\mathrm T}|-\rangle$ contains $|{++}\rangle$’.
Below we develop this idea into a quantum map in ZX-calculus (mainly in the style of PQP).
...
Applying the transpose to $|-\rangle$ (represented as a one-output Z-spider with phase $\pi$ below) gives us a quantum state representing the possible inputs that are mapped to $|-\rangle$ by $\bar f$.
Then we want to check whether this state contains $|{++}\rangle$, but in quantum computing, we have to measure this state with respect to the X-basis (performed by two bastard X-spiders below) before we can check whether the result is ‘${++}$’ or something else.
In ZX-calculus:

![Answering the question](answer.svg)

In general, if a state contains $|{++}\rangle$, we’ll get ‘$++$’ as the result of the measurement only with some probability.
However, our analysis has shown that we’ll definitely get ‘$++$’ in the setting of the algorithm, so the algorithm is deterministic even though measurement is probabilistic in general.

This is not something we can run on a quantum computer yet: the input state $|-\rangle$ and the measurement can be readily implemented as part of a quantum circuit, but the transposed $\hat f$ —and even just $\hat f$ itself— cannot.
...
We want the left part of the diagram to be just two wires connected to the outputs of the transpose of $\hat f$ —which are just the inputs of $\hat f$ because of the ‘directionlessness’— and it’s instinctive for a ZX-calculus user to plug in two one-output Z-spiders (which are in red below, and represent $|+\rangle$)
