...
*Technical remarks.*
As per standard notation, we wrap vectors in ‘kets’ $|\cdot\rangle$, and $|\mathtt{00}\rangle$ etc coincides with the tensor product $|\mathtt 0\rangle \otimes |\mathtt 0\rangle$ etc.
...
To proceed, we need a bit more analysis of $\bar f$ with respect to the whole X-basis, not just $|\texttt{++}\rangle$.
...
$$ \bar f_{\mathsf b}\\;\\_ \approx |\texttt-\rangle \mathrel{\text{or}} 0 $$
...
Even without knowing the ZX-calculus, we know that linear maps (or matrices) can be transposed, thereby switching their domain and codomain, so we can transpose the number $\langle\texttt-|\bar f|\texttt{++}\rangle$ to $\langle\texttt{++}|\bar f^{\mathrm T}|\texttt-\rangle$ and interpret the latter as answering ‘whether $\bar f^{\mathrm T}|\texttt-\rangle$ contains $|\texttt{++}\rangle$’.
...
Then we want to check whether this state contains $|\texttt{++}\rangle$, but in quantum computing, we have to measure this state with respect to the X-basis (performed by two bastard X-spiders below) before we can check whether the result is $\texttt{++}$ or something else.
In ZX-calculus:

![Answering the question](answer.svg)

In general, if a state contains $|\texttt{++}\rangle$, we’ll get $\texttt{++}$ as the result of the measurement only with some probability.
However, our analysis has shown that we’ll definitely get $\texttt{++}$ in the setting of the algorithm, so the algorithm is deterministic even though measurement is probabilistic in general.
