...
To decide whether $f$ is $f_{\mathsf c}$ or $f_{\mathsf b}$, the Deutsch–Jozsa algorithm asks just one clever question, which involves some kind of *backward evaluation* of $f$ on the X-basis: what is a possible input (among the X-basis elements) that allows $f$ to produce an output that ‘contains’ $|-\rangle$?
...
*Remark.*
In fact, the algorithm could even be more simply described as checking whether $\langle-|f|{++}\rangle$ is zero or not.
It’s expensive to compute this number classically, however.
The presentation above is intended to give a better hint about the quantum implementation of the algorithm.
(*End of remark.*)
