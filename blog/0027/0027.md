...
The Deutsch–Jozsa problem is (artificially) designed to make the sums useful: assuming that $f$ is either *constant* (always producing the same output) or *balanced* (producing two $|0\rangle$ and two $|1\rangle$ as the outputs — in general the same number of $|0\rangle$ and $|1\rangle$), the aim is to decide which is the case.
