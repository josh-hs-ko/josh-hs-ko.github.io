...
It is immediately clear that a constant $f$ —let’s write this as $f_{\mathsf c}$— behaves differently from a balanced $f$ —let’s write this as $f_{\mathsf b}$— when applied to $|{++}\rangle$:
...
At this point, it’s actually easy to decide whether $f$ is $f_{\mathsf c}$ or $f_{\mathsf b}$ by asking this question: if we apply $f$ to $|{++}\rangle$, will the output ‘contain’ $|-\rangle$?
...
Then we measure this state with respect to the X-basis (performed by two bastard X-spiders below), so that we can check whether the result is ‘${++}$’ or something else.
In ZX-calculus:
...
Note that this is merely a mathematical specification and not a quantum circuit, but the construction does give a legitimate quantum circuit with equivalent behaviour.
